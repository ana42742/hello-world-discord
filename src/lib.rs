...
async fn handler(bot: &ProvidedBot, msg: Message) {
    ...
    if user_message.contains("hello") {
        let response = "Hello there! How can I help you today?";
        send_response_to_discord(&discord, channel_id, response).await;
    } else if user_message.contains("explain image segmentation") {
        let response = "Image segmentation is a computer vision task that divides an image into meaningful regions,
        ...
        let response = "FFmpeg is a powerful open-source multimedia framework that facilitates audio and video processing,
        ...
        let response = "WasmEdge is an open-source, high-performance runtime for WebAssembly (Wasm) designed for edge
        ...
        let response = "An object detection image processing pipeline involves several steps. First, preprocess the image
