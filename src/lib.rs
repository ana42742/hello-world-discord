...
async fn handler(bot: &ProvidedBot, msg: Message) {
    ...
    if user_message.contains("hello") {
        ...
        response = "WasmEdge is an open-source, high-performance runtime for WebAssembly (Wasm) designed for edge computing and the Internet of Things (IoT). It executes Wasm modules securely and efficiently, enabling seamless integration of multiple programming languages. (Reference: WasmEdge GitHub repository - https://github.com/WasmEdge/WasmEdge)";
    } else if user_message.contains("object detection") {
        response = "An object detection image processing pipeline involves several steps. First, preprocess the image by resizing and normalizing it. Then, extract relevant features through techniques like edge detection. Next, propose potential object regions using methods such as selective search. Extract features from these regions and classify them using a model like a Convolutional Neural Network (CNN). Apply non-maximum suppression to eliminate overlapping detections. Refine the results through post-processing, filtering out false positives. Finally, visualize the detected objects by drawing bounding boxes. Modern methods integrate these steps into deep learning networks for more accurate and efficient object detection, such as YOLO and Faster R-CNN.";
